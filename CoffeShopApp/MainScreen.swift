...
struct MainScreen: View {
    ...
    @State private var orderedDrink: [Item] = []
    @State private var orderedPastry: [Item] = []
                ...
                ScrollViewReader { proxy in
                    ScrollView(.vertical) {
                        VStack () {
                            ...
                            HStack {
                                Button {
                                    proxy.scrollTo("drinkSectionId", anchor: .top)
                                } label: {
                                    ZStack {
                                    ...
                                    Button {
                                        proxy.scrollTo("pastrySectionId", anchor: .top)
                                ...
                                NavigationLink {
                                    CartView(orderedDrink: $orderedDrink, orderedPastry: $orderedPastry, total: 3) } label: {
                                        ZStack {
                                            RoundedRectangle(cornerRadius: 10)
                                                .frame(width: 85, height: 50)
                                                .foregroundStyle(.sirenRed)
...
//                                        }
                            ...
                            VStack (alignment: .leading) {
                                ...
                                VStack(alignment: .leading){
                                    ...
                                    NavigationLink {
                                        PopularItemsView(orderedDrink: $orderedDrink)
                                    } label: {
                                        ImageSliderView()
                                    }
                                    ...
                                    VStack(alignment: .leading) {
                                        ...
                                        HStack {
                                            ForEach(whenSearching.filter { $0.itemType == .pastry }, id:\.self) { item in
                                                NavigationLink {
                                                    PastryCustomizationView(orderedPastry: $orderedPastry, item: item)
                                                } label: {
                                                    itemTileView(item: item)
                                                }
                                            }
...
//                                .accessibilityHint("This button will take you to a page giving details about the cafe like address, phone number and opening date.")
                                ...
                                HStack {
                                    ...
                                    Link(destination: URL(string: "https://www.facebook.com/photo.php?fbid=1541002580593936&id=100040526276371&set=a.732405361453666")!) {
                                        Image(.facebook1)
                                            .resizable()
                                            .scaledToFit()
                                    ...
                                    Link(destination: URL(string: "https://www.instagram.com/thesirenhotel/")!) {
                                        Image(.instagram)
                                            .resizable()
                                            .scaledToFit()
                                            .frame(width: 40, height: 40)
